...
public class TasksFrom16To20 {
    ...
    static Boolean spidersBiteMore(List<Animal> animals) {
        return
            animals.stream().filter(animal -> animal.type() == Animal.Type.SPIDER || animal.type() == Animal.Type.DOG)
                .collect(Collectors.groupingBy(
                    Animal::type,
                    Collectors.summingInt(animal -> (animal.bites() != null && animal.bites()) ? 1 : 0)
                )).getOrDefault(Animal.Type.SPIDER, 0)
                > (Integer) animals.stream().filter(animal -> animal.type() == Animal.Type.DOG)
    ...
    static Animal heaviestFish(List<Animal> animals1, List<Animal> animals2) {
        return Stream.of(animals1, animals2).flatMap(List::stream).filter(animal -> animal.type() == Animal.Type.FISH)
    ...
    static Map<String, Set<ValidationError>> animalsWithErrors(List<Animal> animals) {
        return animals.stream()
            ...
            .collect(Collectors.toMap(Animal::name,
                animal -> Stream.of(animal.name() == null ? ValidationError.NAME_IS_NULL : null,
    ...
    static Map<String, String> animalsWithErrorsReadable(List<Animal> animals) {
        return animalsWithErrors(animals).entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey,
            entry -> entry.getValue().stream().map(Enum::toString).collect(Collectors.joining(", "))
        ));
    }
